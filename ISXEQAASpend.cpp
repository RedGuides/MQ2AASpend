...
#include "ISXEQAASpend.h"

// The mandatory pre-setup function.  Our name is "ISXEQAASpend", and the class is ISXEQAASpend.
...
// our instanced class.
ISXPreSetup("ISXEQAASpend",ISXEQAASpend);
...
unsigned int ISXEQAASpendXML=0;
...
bool ISXEQAASpend::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQAASpend Loaded");
return true;
}

// shutdown sequence
void ISXEQAASpend::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQAASpendXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQAASpendXML);
...
void ISXEQAASpend::ConnectServices()
{
...
void ISXEQAASpend::RegisterCommands()
{
    // add any commands
...
void ISXEQAASpend::RegisterAliases()
{
    // add any aliases
}

void ISXEQAASpend::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQAASpend::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQAASpend::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQAASpend::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQAASpend::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQAASpend::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQAASpendService)
    //        pISInterface->ShutdownService(this,hISXEQAASpendService);
}

void ISXEQAASpend::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQAASpendXML
    ISXEQAASpendXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQAASpendXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQAASpendXML,"General"))
    {
